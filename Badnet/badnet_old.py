...
class PoisonedMNIST(Dataset):
    def __init__(self, dataset, poison_indices=None, target_label=TARGET_LABEL):
...
def visualize_poisoned_samples(dataset, filename="poisoned_samples.png", max_samples=25):
...
def main():
    ...
    clean_test = PoisonedMNIST(test_set)
    poisoned_test = PoisonedMNIST(test_set, poison_indices_test)
    visualize_poisoned_samples(poisoned_test)
